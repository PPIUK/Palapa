  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
        ...
        <Text style={styles.subtitle}>by {event.penyelenggara}</Text>

        <View style={styles.cardContainer}>
          <View style={styles.desc}>
            <Feather name="calendar" size={24} color="#004380" />
            <Text style={{ marginHorizontal: 10 }}>
              {event.date.slice(0, 10)} {event.date.slice(30)}
            </Text>
          </View>
          <View style={styles.desc}>
          ...
          <View style={styles.desc}>
            ...
            {event.link.slice(0, 4) !== "http" ? (
              ...
              >
                <Text
                  style={{ color: "#004380", textDecorationLine: "underline" }}
                >
                  {event.link}
                </Text>
              </TouchableOpacity>
        ...
        <Text style={{ margin: 20, fontSize: 22 }}>{event.desc}</Text>
        {event.registrasi ? (
          <TouchableOpacity
            style={styles.button}
            onPress={() =>
              navigation.navigate("EventLink", { link: event.registrasi })
            }
          >
