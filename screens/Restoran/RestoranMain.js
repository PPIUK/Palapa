...
const ppi = [{ value: 0, label: "All City" }];
...
export default function RestoranMain({ navigation }) {
  ...
  function getDataFromAPI() {
    dataAPI
      .get("app/application-0-jjbbw/endpoint/HTTP/get_restoran_katering")
  ...
  function filterList(Text) {
    if (Text !== "") {
      setfilteredData(data.filter((x) => x.name.toLowerCase().includes(Text.toLowerCase())));
    } else {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Text style={styles.title}>Restoran & Katering</Text>
        ...
        <Image
          style={styles.banner}
          source={require("../../assets/restoran.jpg")}
        />

        <Text style={styles.subtitle}>Cari berdasarkan kota:</Text>
        ...
        {restolist(value).map(({ _id, name, city }, index) => {
          return (
            <TouchableOpacity
              key={index}
              onPress={() => {
                navigation.navigate("DetailRestoran", {
                  resto: restolist(value)[index],
                });
              }}
