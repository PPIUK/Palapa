...
function loop({
  ...
  return _reactNative.Animated.loop(timing);
}
...
function stagger(time, amount, animationConfig) {
  ...
  if (_reactNative.Platform.OS === 'web') {
    const values = new Array(amount).fill(null).map(_ => new _reactNative.Animated.Value(0));
    const animations = values.map(value => loop({
      value,
      duration,
      easing,
      toValue,
    ...
    const animation = _reactNative.Animated.stagger(time, animations);
  ...
  const animation = _reactNative.Animated.loop(timing); // React Native only does 60fps
